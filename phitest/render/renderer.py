...
from .cuda.ops_loader import (  # sample_grid_transform, sample_grid_lut, reduce_grid_blend, raymarch_grid_transform
    blending_ops,
    raymarching_ops,
    sampling_ops,
)
...
class RenderingContext:
    ...
    def randomize_camera_rotation(self, x_range=None, y_range=None, z_range=None):
        if x_range is None:
            x_range = self.cam_x_range
        if y_range is None:
            y_range = self.cam_y_range
        if z_range is None:
            z_range = self.cam_z_range
...
class Renderer(object):
    ...
    def get_camera_LuT(self, grid_transform, cam, inverse=False):
        ...
        LuTs = sampling_ops.lod_transform(
    ...
    def __get_cam_luts(t, size, cams, inverse):
        if inverse:
            setup_cams = [cam for cam in cams if cam.inverseLuT is None]
        else:
            setup_cams = [cam for cam in cams if cam.LuT is None]

        if len(setup_cams) > 0:
            self._setup_static_camera_LuT(transformation, (size, setup_cams), inverse)

        if inverse:
            luts = [cam.inverseLuT for cam in cams]
        else:
            luts = [cam.LuT for cam in cams]
    ...
    def check_LoD(self, grid_transform, camera, check_inverse=True, name=None):
        ...
        if check_inverse:
            ...
            if name is not None:
                self.log.info(
                    "%s stats: shape: %s (%.2f Mi), step: %f, LoD: %f - %f (grad: %f - %f)",
                    name,
                    camera.transform.grid_size,
                    np.prod(camera.transform.grid_size) / (1024 * 1024),
                    camera.depth_step,
                    LoD_min,
                    LoD_max,
                    LoD_grad_min,
            ...
            stats = {
                "shape": camera.transform.grid_size,
                "step": camera.depth_step,
                "LoD_min": LoD_min,
                "LoD_max": LoD_max,
                "LoD_grad_min": LoD_grad_min,
                "LoD_grad_max": LoD_grad_max,
            }
        else:
            if name is not None:
                self.log.info(
                    "%s stats: shape: %s (%.2f Mi), step: %f, LoD: %f - %f",
                    name,
                    camera.transform.grid_size,
                    np.prod(camera.transform.grid_size) / (1024 * 1024),
                    camera.depth_step,
        ...
        with self.profiler.sample("sample_transform"):
            ...
            if fix_scale_center:
                V = [
                    (
                        GridTransform(
                            t.grid_size, scale=[2, 2, 2], center=True, normalize="ALL"
                        ).get_transform_matrix()
                        @ t.get_inverse_transform()
                    ).transpose()
                    for t in to_transformations
                ]
            else:
            ...
            if self.sample_gradients:
                ...
                def __sample(x, m, v, p, f, out_shape):  # NDHWC, N44, V44, V44, V6, 3(:DHW-out)
                    with self.profiler.sample("Sampling kernel"):
                        y = sampling_ops.sample_grid_transform(
                            input=x,
                            matrix_m=m,
                            matrix_v=v,
                            matrix_p=p,
                            frustum_params=f,
                            output_shape=out_shape,
                            interpolation=self.filter_mode,
                            boundary=self.boundary_mode,
                            mipmapping=self.mip_mode,
                            num_mipmaps=self.num_mips,
                            mip_bias=self.mip_bias,
                    ...
                    def grad(dy):  # NVDHWC
                        with self.profiler.sample("Sampling gradients kernel"):
                            ...
                            for i in range(batch):  # iterate batch (N)
                                ...
                                dx_views = sampling_ops.sample_grid_transform(
                                    input=dy[i],
                                    matrix_m=m_views,
                                    matrix_v=v,
                                    matrix_p=p,
                                    frustum_params=f,
                                    output_shape=in_shape[-4:-1],
                                    interpolation=self.filter_mode,
                                    boundary=self.boundary_mode,
                                    mipmapping=self.mip_mode,
                                    num_mipmaps=self.num_mips,
                                    mip_bias=self.mip_bias + self.gradient_mip_bias_add,
                                    coordinate_mode=CM[not inverse],
                                    separate_camera_batch=False,
                                )  # V1DHWC
            ...
            with self.profiler.sample("Sampling kernel"):
                if self.sample_gradients:
                    sampled = __sample(
                        ...
                        P,
                        F,
                        from_transformations[0].grid_size if inverse else to_transformations[0].grid_size,
                    )
                else:
                    sampled = sampling_ops.sample_grid_transform(
                        input=data,
                        matrix_m=M,
                        matrix_v=V,
                        matrix_p=P,
    ...
    def _sample_camera_transform(self, data, transformations, cameras, inverse=False):
        with self.profiler.sample("sample_camera_transform"):
            ...
            with self.profiler.sample("Sampling kernel"):
                if self.sample_gradients:
                    sampled = __sample(
                        data, M, V, P, F, transformations[0].grid_size if inverse else cameras[0].transform.grid_size
                    )
                else:
                    sampled = sampling_ops.sample_grid_transform(
                        input=data,
                        ...
                        matrix_p=P,
                        frustum_params=F,
                        output_shape=transformations[0].grid_size if inverse else cameras[0].transform.grid_size,
                        interpolation=self.filter_mode,
                        boundary=self.boundary_mode,
                        mipmapping=self.mip_mode,
                        num_mipmaps=self.num_mips,
        ...
        with self.profiler.sample("sample_camera_LuT"):
            ...
            if self.sample_gradients:
                ...
                def __sample(x, out_shape):  # NDHWC, N44, V44, V44, V6, 3(:DHW-out)
                    ...
                    with self.profiler.sample("Sampling kernel"):
                        y = sampling_ops.sample_grid_lut(
                            input=x,
                            lookup=luts,
                            interpolation=self.filter_mode,
                            boundary=self.boundary_mode,
                    ...
                    def grad(dy):  # NVDHWC
                        with self.profiler.sample("Sampling gradients kernel"):
                            ...
                            if tf.shape(dy)[0].numpy() != 1:
                                raise NotImplementedError(
                                    "camera lut rendering does not support data batches.", tf.shape(dy)[0].numpy()
                                )
                            dy = dy[0]  # N - VDHWC
                            luts = self.__get_cam_luts(transformation, out_shape, cameras, not inverse)
                            dx = sampling_ops.sample_grid_lut(
                                input=dy,
                                lookup=luts,
                                interpolation=self.filter_mode,
                                boundary=self.boundary_mode,
            ...
            with self.profiler.sample("Sampling kernel"):
                if self.sample_gradients:
                    ...
                    sampled = sampling_ops.sample_grid_lut(
                        input=data,
                        lookup=luts,
                        interpolation=self.filter_mode,
                        boundary=self.boundary_mode,
    ...
    def _raymarch_camera_transform(self, data, transformations, cameras, **kwargs):
        with self.profiler.sample("raymarch_camera_transform"):
            ...
            with self.profiler.sample("Sampling kernel"):
                sampled = raymarching_ops.raymarch_grid_transform(
                    input=data,
                    matrix_m=M,
                    matrix_v=V,
                    matrix_p=P,
    ...
    def sample_camera(
        ...
        allow_static=True,
        force_static=False,
        use_step_channel=None,
    ):
        # check data
        data_shape = tf.shape(data).numpy()
        if not len(data_shape) == 5:
            raise ValueError("data must be 5D (NDHWC)")
        if not isinstance(transformations, Iterable):
            ...
            if len(transformations) != data_shape[0]:
                raise ValueError("transformation and data batch size mismatch.")
            no_batch = False

        # check cameras
        if not isinstance(cameras, Iterable):
            cameras = [cameras]  # compat
        if len(cameras) > 1 and not all(
            (cam.transform.grid_size == cameras[0].transform.grid_size for cam in cameras[1:])
        ):
            raise ValueError(
                "all cameras must have the same resolution (DHW). (use Renderer._sort_cameras() for batching.)"
            )
        ...
        if allow_static and any((cam.static is not None for cam in cameras)):
            if not all((cam.static == transformations[0] for cam in cameras)):
                if force_static:
                    raise ValueError("Camera static setup does not match transformation")
                else:
                    self.log.warning(
                        "Incorrect static camera setup, falling back to transform rendering for static cameras."
                    )
            else:
                if not no_batch:
                    if force_static:
                        raise ValueError("Static cameras only work without data batch.")
                    else:
                        self.log.warning(
                            "Incorrect static camera setup, falling back to transform rendering for static cameras."
                        )
        ...
        if use_step_channel is not None and use_step_channel != []:
            if np.isscalar(use_step_channel):
                ...
                step_channel = [
                    _ % data_shape[-1] for _ in use_step_channel if ((-data_shape[-1]) <= _ and _ < data_shape[-1])
                ]
                ...
                if np.all([step == depth_steps[0] for step in depth_steps]):
                    ...
                    # 	else -> premultiply with channel vector
                        ...
                        data = data * tf.constant(
                            [(depth_steps[0] if _ in step_channel else 1) for _ in range(data_shape[-1])],
                            dtype=tf.float32,
                        )
        ...
        if apply_step_channel:
            ...
            step = tf.constant(
                [
                    [(depth_step if _ in step_channel else no_step) for _ in range(shape[-1])]
    ...
    def _sample_LuT(self, data, luts, combined_batch=False, relative=False, normalized=False, cell_center_offset=0.0):
        with self.profiler.sample("sample_LuT"):
            ...
            if luts_shape.c != 4:
                raise ValueError
            with self.profiler.sample("Sampling kernel"):
                resampled = sampling_ops.sample_grid_lut(
                    input=data,
                    lookup=luts,
                    interpolation=self.filter_mode,
                    boundary=self.boundary_mode,
                    ...
                    mip_bias=self.mip_bias,
                    coordinate_mode="LOOKUP",
                    separate_camera_batch=not combined_batch,
        ...
        with self.profiler.sample("_blend_grid"):
            ...
            if blend_mode.upper() == "MAX":
                ...
                if grid_shape.c == 1:  # blending for density only
                    return (
                        tf.math.cumsum(data, axis=-4, exclusive=False) if keep_dims else tf.reduce_sum(data, axis=-4)
                    )
                ...
                if keep_dims:
                    return tf.concat(
                        [tf.math.cumsum(light * tf.math.exp(-dens_sum), axis=-4, exclusive=False), dens_sum], axis=-1
                    )
                else:
                    return tf.concat(
                        [tf.reduce_sum(light * tf.math.exp(-dens_sum), axis=-4), dens_sum[..., -1, :, :, :]], axis=-1
                    )
                ...
                if keep_dims:
                    ...
                    return tf.concat(
                        [
                            tf.reduce_sum(light * tf.math.exp(-dens_sum), axis=-4),
                            tf.reduce_sum(density, axis=-4, keepdims=keep_dims),
                        ],
                        axis=-1,
                    )
    ...
    def _render_cameras(self, light_density, grid_transform, cameras, camera_size, custom_ops=None):
        ...
        if self.can_render_fused and ((custom_ops is None) or ("FRUSTUM" not in custom_ops)):
            with self.profiler.sample("Sampling & Reduction"):
                image_hdr = self.raymarch_camera(
                    light_density,
                    grid_transform,
                    cameras,
                    use_step_channel=[0, 1, 2, 3] if self.blend_mode not in ["MAX", "MEAN", "MIN"] else None,
                )
        else:
            with self.profiler.sample("Sampling"):
                frustum_grid = self.sample_camera(
                    light_density,
                    grid_transform,
                    cameras,
                    False,
            ...
            for light in light_list:
                if isinstance(light, Light):
                    with self.profiler.sample("Light {}: {}".format(i, type(light).__name__)):
                        light_grid = light.grid_lighting(
                            density_transform, self
                        )  # scattering_func=self._volume_scatter
                ...
                for cam_size, cams in cameras:
                    with self.profiler.sample("Size {} x{}".format(cam_size, len(cams))):
                            ...
                            for cam in cams:
                                images.append(
                                    self._render_cameras(
                                        data, density_transform, [cam], cam_size, custom_ops=custom_ops
                                    )
                                )
                            images = tf.concat(images, axis=0)
                        else:
                            images = self._render_cameras(
                                data, density_transform, cams, cam_size, custom_ops=custom_ops
                            )
                        ...
                        if background is not None:
                            cam_batch_bkgs = tf.stack(
                                [tf.broadcast_to(background[camera_list.index(cam)], img_shape[1:]) for cam in cams]
                            )
    ...
    def unproject(self, grid_transform, targets, cameras, blend_func=tf.minimum):
        ...
        if self.allow_fused and False:
            ...
            unprojections = raymarching_ops.raymarch_grid_transform_grad(
                input=inp,
                output=tar,
                output_grad=tar,
                matrix_m=M,
    ...
    def visual_hull(
        self, grid_transform, targets, cameras, image_blur=0.0, grid_blur=0.0, threshold=0.5, soft_blur=0.0
    ):

        target_hulls = (
            tf_data_gaussDown2D(targets, image_blur, stride=1, channel=1, padding="SAME")
            if image_blur > 0
            else targets
        )
            ...
            for image in image_batch:
                ...
                if format == "EXR":
                    try:
                        ...
                        self.log.exception(
                            "Failed to write exr image with shape %s to '%s':", image.get_shape().as_list(), path
                        )
                        return
                elif format == "PNG":
                    ...
                    try:
                        ...
                        self.log.exception(
                            "Failed to write png image with shape %s to '%s':", image.get_shape().as_list(), path
                        )
                        return
                else:
