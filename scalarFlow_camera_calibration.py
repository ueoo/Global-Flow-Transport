...
idx = [
    (539, 959),
]  # ,(0, 959), (1079, 959), (539, 0), (539, 1919)]


cams = []
for i in [1, 2, 3, 4, 5]:
    rays = pd.read_csv(
        "data/scalarFlow/calib20190813/{}_rays.txt".format(i),
        sep=" ",
        skiprows=1,
        header=None,
        names=["pY", "pX", "dY", "dX", "dZ"],
        index_col=(0, 1),
    )
    cam = {}
    for key, idx in indices.items():
        ...
        try:
            for id in idx:
                ...
                tmp.append(
                    {
                        "start": np.asarray([ray["pX"], ray["pY"], 0.0]),
...
def dist_to_ray(pos, start, dir):
    dist = np.linalg.norm(to_ray(pos, start, dir))
...
for i in range(len(cams)):
    ...
    print(
        "\tangles: zx={}, zy={}, xy={}".format(
            angle_between(fwd, right) * rad_to_deg,
            angle_between(fwd, up) * rad_to_deg,
            angle_between(right, up) * rad_to_deg,
        )
    )
...
cam_json["fov_horizontal_average"] = np.mean(
    [cam_json[str(i)]["fov_horizontal"] for i in cam_order if cam_json[str(i)]["fov_horizontal"] is not None]
)
cam_json["fov_vertical_average"] = np.mean(
    [cam_json[str(i)]["fov_vertical"] for i in cam_order if cam_json[str(i)]["fov_vertical"] is not None]
)
...
cam_json["volume_offset"] = [
    cam_json["marker_width"] / 6.0,
    -cam_json["marker_width"] / 11.0,
    -cam_json["marker_width"] / 100.0,
]  # world-space position of the (0,0,0) corner of the volume
# cam_json['volume_size'] = [cam_json['volume_width'], math.ceil(cam_json['scale_y']*cam_json['volume_width']), cam_json['volume_width']]
cam_json["volume_size"] = [
    cam_json["volume_width"],
    cam_json["scale_y"] * cam_json["volume_width"],
    cam_json["volume_width"],
]
